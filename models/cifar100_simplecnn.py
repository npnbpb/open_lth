...
class Model(base.Model):
    '''A simple CNN for CIFAR-100'''

    def __init__(self, channels, initializer, outputs=100):
        ...
        final_size = int(floor(32/(2**len(channels)))**2 * out_features)
    ...
    def is_valid_model_name(model_name):
        return (model_name.startswith('cifar100_simplecnn') and
    ...
    def get_model_from_name(model_name, initializer, outputs=None):
        """The name of a model is cifar100_simplecnn_N1[_N2...].

        N1, N2, etc. are the number of features in each convolutional layer excluding the
        output layer (100 neurons by default). A simplecnn with 300 features in the first conv layer,
        100 feature in the second conv layer, and 100 output neurons is 'cifar100_simplecnn_300_100'.
        """
    ...
    def default_hparams():
        model_hparams = hparams.ModelHparams(
            model_name='cifar100_simplecnn_16_32',
